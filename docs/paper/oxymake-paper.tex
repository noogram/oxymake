...
job graph in 118\,ms against Snakemake~7.32.4's 2.72\,s, a 23.0$\times$
...
\midrule
101      &    6\,ms  &  595\,ms     & 100.86$\times$ \\
1{,}001  &   13\,ms  &  792\,ms     & 60.49$\times$ \\
10{,}001 &  118\,ms  & 2{,}720\,ms  & 23.01$\times$ \\
\bottomrule
\end{tabular}
...
\noindent OxyMake resolves a 10{,}001-job DAG in 118\,ms versus
Snakemake's 2.72\,s, a 23.01$\times$ difference on DAG resolution. The ratio
...
the small rows: a hundredfold increase in job count, from 101 to
1{,}001 to 10{,}001, moves \code{snakemake --dryrun} only from 595\,ms
to 792\,ms to 2.72\,s, so most of the 101-job row's 595\,ms is fixed
interpreter startup and import rather than resolution work, and the
100.86$\times$ entry should not be read as one. The
...
\midrule
101      &  1.62\,s  &   2.39\,s  & 1.47$\times$ (slower) \\
1{,}001  &  6.75\,s  &  17.37\,s  & 2.57$\times$ (slower) \\
10{,}001 & 92.67\,s  & 196.93\,s  & 2.13$\times$ (slower) \\
\bottomrule
\end{tabular}
...
\begin{itemize}
  \item \textbf{Warm-cache re-run} (the common inner-loop case): at 10{,}001 jobs,
    ...
    rebuild in 1.38\,s under the shipped \code{mtime+hash} default
    versus Snakemake's 3.48\,s---2.52$\times$ faster. Pinning
    \code{--cache-validation=mtime}, the metadata fast path, brings the
    same no-op rebuild to 500\,ms---6.95$\times$ faster---and
    \code{--cache-validation=hash}, which re-hashes every input and
    output before deciding, to 1.82\,s---1.91$\times$ faster, with
...
$101 \to 10{,}001$ jobs in Table~\ref{tab:dag-resolution} (6\,ms~$\to$~13\,ms~$\to$~118\,ms:
...
\midrule
DAG resolution @ 1{,}001 jobs   & 13\,ms \\
DAG resolution @ 10{,}001 jobs  & 118\,ms \\
DAG res. speedup @ 10{,}001 jobs & 23.01$\times$ \\
Cold end-to-end @ 10{,}001 jobs & 2.13$\times$ slower \\
Warm no-op @ 10{,}001 jobs (\code{mtime+hash}) & 2.52$\times$ faster \\
Peak RSS, cold @ 10{,}001 jobs & 89.9\,MiB (2.05$\times$ smaller) \\

...
(\S\ref{sec:agent-api}). An MCP server (\code{ox serve --mcp}) exposes a
read-only inspection subset of the engine over the Model Context
Protocol. This is an
...
measured: Snakemake~7.32.4 re-runs zero jobs, because its recorded
provenance ignores live timestamps, and a pure-mtime policy re-runs the
full downstream radius. The third is true by construction and the
benchmark only confirms it: under the \code{mtime+hash} default a
...
the default (\S\ref{sec:file-based}); Bazel, Buck2, and Nix take the
daemon or the store as an operating premise. (DVC is the acknowledged
...
\item \textbf{A machine-readable execution interface.} Structured
  ...
  server exposes a read-only inspection subset of these operations
  (\S\ref{sec:agent-api}).
...
Startup time (\code{ox --help}) & 6\,ms \\

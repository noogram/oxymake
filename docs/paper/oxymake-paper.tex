...
(7.54$\times$ under \code{mtime}, 4.02$\times$ under \code{hash};
both engines at 16-way parallelism) (\S\ref{sec:e2e-bench}). Because concurrent \code{ox run} processes
...
\item \code{mtime} (opt-in) --- pure filesystem metadata (stat calls only).
  ...
  with workflow size: sub-100\,ms on small workflows, 372\,ms at
  10{,}001 jobs (\S\ref{sec:e2e-bench}). Content is never verified, so this
...
\code{--cache-validation=mtime}, the
metadata fast path (\S\ref{sec:content-addressable}); an additional
warm run uses \code{--cache-validation=hash}, full content
re-verification. The \code{mtime} rows were produced by the default in
force at measurement time; the shipped default is now
\code{mtime+hash}, which re-hashes a file whose metadata moved and so
does not follow the metadata fast path on a warm re-run. The harness
now pins the validation policy on each row. The
...
while OxyMake's resolution grows linearly. That fixed cost dominates
the small rows: \code{snakemake --dryrun} on a two-rule, one-job
workflow on this host family takes 0.40--0.45\,s, so the 101-job row's
418\,ms is essentially all interpreter startup and import, and the
101.9$\times$ entry is not a measurement of resolution work. The
marginal ratio at 10{,}001 jobs---derived from a linear fit through the
two endpoints---is $\approx$29$\times$. From 101 to 10{,}001 jobs ($\approx$100$\times$
...
\begin{itemize}
  \item \textbf{Warm-cache re-run} (the common inner-loop case): at 10{,}001 jobs,
    ...
    rebuild in 372\,ms under \code{--cache-validation=mtime} versus
    Snakemake's 2.81\,s---7.54$\times$ faster. Under
    \code{--cache-validation=hash}, which
    ...
    every byte verified. The shipped \code{mtime+hash} default sits
    between the two and closer to \code{hash}, because it re-hashes any
    file whose metadata moved.
...
is not a community
standard like CWL, though the \code{ox translate} command provides
conversion in both directions with Snakemake and WDL as bridges to the

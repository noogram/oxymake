...
| ADR-006 | Pluggable Cache Validation Strategies | Accepted — **amended 2026-06-10: default changed from `mtime` to `mtime+hash`** | ADR-001, ADR-018, ADR-019 | — | blake3, cache_validation, mtime |
...
| ADR-015 | Named Invariants and TLA+ Scope | Accepted (revised from the D2 scope decision by a follow-up design panel; | ADR-019 | — | cancel, session, sqlite, state_db |
